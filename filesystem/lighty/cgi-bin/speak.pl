...
#    my $rc = system("echo /mnt/usb/TalkingChumby/time.sh \"${hour}:${min}\" ");
    my $rc = system("/mnt/usb/TalkingChumby/time.sh \"${hour}:${min}\" ");
...
if ($in{'action'} eq "time") { #say the time HH:MM 
    my $rc = system("/mnt/usb/TalkingChumby/time.sh $in{'time'}");
...
if ($in{'action'} eq "say") { #say the words 
    my $rc = system("/mnt/usb/TalkingChumby/say.sh \"$in{'words'}\"");
