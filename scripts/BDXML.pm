...
#        print STDERR "\n";
        ...
        if( (($c eq '"') && ($state == 3)) 
         || (($c eq "'") && ($state == 4)) ) {
            $state = 9;
            $$properties{$currentProperty} = html_unescape($currentString);
        ...
        if( $c eq '<' ) {
            ...
            if( length($currentString) > 0 ) {
                $value = html_unescape($currentString);
                $currentString = "";
            }
...
#                    print STDERR "Recursing downwards\n";
                    unget_char($c);
                    my $child = parse();
                    push(@$children, html_unescape($child));
                    $state = 0;
...
sub unparse {
    ...
    if(defined($$node{'properties'})) {
        while( my ($key, $value) = each %{$$node{'properties'}} ) {
            $str .= " $key=\"" . html_escape($value) . "\"";
        }
    }
    if(defined($$node{'value'})) {
        $str .= ">" . html_escape($$node{'value'}) . "</" . $$node{'name'} . ">";
    }
